          ...
          <div class="col-sm-12 p-md-0">
            ...
            <div class="row">
              <div class="welcome-text col-lg-6">
                <h2 style='color:#007bac'>Liste des locataires</h2>
               </div>
              <div class="col-lg-6">

                <ul class="nav nav-pills nav-fill justify-content-around rounded shadow m-4">
                  <li class="nav-item"><a class="nav-link <?php if(in_array($this->router->method,['insert'])) echo 'active';?>"  href="<?=base_url('locataire/Locataire/Add/')?>"><i class="fa fa-pencil-square-o"></i>Nouveau</a></li>
...
<script>
  ...
  function liste()
  {
   ...
   table=$("#mytable").DataTable({
    ...
    "oreder":[[ 0, 'desc' ]],
    "ajax":{
      url:"<?=base_url()?>locataire/Locataire/listing/",
      type:"POST"
    },
...
// Réinitialiser le formulaire
  ...
  $('.help-block').empty();
    ...
    $.ajax({
      url : "<?php echo site_url('locataire/Locataire/getOnetraite')?>/" + id,
      type: "GET",
      dataType: "JSON",
      success: function(data)
      {

        $('[name="ID_LOCATAIRE"]').val(data.ID_LOCATAIRE);
        $('[name="ID_STATUT_DEMANDE"]').html(data.ID_STATUT_DEMANDE);
  ...
  function traitement()
  {
   ...
   $.ajax({
    ...
    url:"<?php echo base_url('locataire/Locataire/traite_ticket')?>",
    type:"POST",
    data:formData,
    contentType:false,
    processData:false,
 ...
 function get_statut(id){
  ...
  $.post('<?=base_url('locataire/Locataire/get_statut')?>',
  {
    id:id
  },
...
{
  $.post('<?=base_url('locataire/Locataire/get_name/')?>',
  {
    id:id
  },
  function(data)
...
function historique_ticket(id)
{
  ...
  table1=$("#table-historique").DataTable({
    ...
    "oreder":[[ 6, 'desc' ]],
    "ajax":{
      url: "<?php echo base_url('locataire/Locataire/historique/');?>"+id, 
      type:"POST",
      beforeSend : function() {
      }
    },
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title-plainte">Traitement de la demande  </b></h5>
        ...
        <div class="row">
          ...
          <div class="col-12">
           <form id="form_statut">
            <div class="form-body">
              <!-- <div class="row"> -->
                <input type="hidden" name="ID_LOCATAIRE" id="ID_LOCATAIRE">
                <div class="col-md-12">
                  <div class="form-group ">
                    <label>Statut de la demande </label>
                    <select class="form-control" name="ID_STATUT_DEMANDE" id="ID_STATUT_DEMANDE" >
                      <option value="">Choisir</option>
                      <?php
                      foreach ($statut as $key) 
                      {
                        echo "<option value=".$key['ID_STATUT_DEMANDE'].">".$key['DESC_DEMANDE']."</option>";
                      }
                      ?>
                    </select>
...
<div class="modal fade" data-backdrop="static" id="liste-historique">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Historique de traitement de la demande  </h5>
        <button type="button" class="close" data-dismiss="modal"><span>&times;</span>
        </button>
      </div>
      <div class="modal-body">
